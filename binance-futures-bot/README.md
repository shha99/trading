...
- **BTCUSDT 15분봉·5분봉**: `bollinger_wick_breakeven_trail`(볼린저 꼬리터치
  ...
  - BTC 15분봉: 8,488건, 학습 88.0%/+1.22%, 검증 75.8%/+0.19%
  - BTC 5분봉: 14,515건, 학습 77.5%/+0.51%, 검증 55.8%/+0.09% (넷 중 마진이 가장 얇음)
  - ETH 15분봉(교차검증): 9,110건, 학습 83.7%/+1.09%, 검증 78.3%/+0.42%
  - ETH 5분봉(교차검증): 23,075건, 학습 71.9%/+0.24%, 검증 63.7%/+0.11%

  BTC뿐 아니라 ETH·15분·5분봉·5년 이상 전체 구간에서 수수료를 반영하고도
  전부 학습/검증 둘 다 플러스라, 콘플루언스 전략보다 훨씬 폭넓게 검증됐다.
  다만 이 파라미터(손절 3×ATR·본전이동 +0.5×ATR·트레일 0.3×ATR)는 **원래
  수수료 0원으로 가정하고 찾은 조합(손절 2×ATR·본전이동 +0.3×ATR·트레일
  0.3×ATR)을 실제 수수료 반영 후 재탐색해서 바꾼 값**이다 — 처음 조합은
  트레일이 타이트해서 대부분의 익절이 1% 미만으로 끝났는데, 수수료
  0.1%만 반영해도 "이겼는데 수수료로 진" 거래가 60~76%나 나왔고
  (특히 5분봉), 스트레스 가정인 1% 수수료에서는 네 조합 전부 검증구간
  승률이 3~16%까지 붕괴했다. 손절을 넉넉히·본전이동을 늦게 잡아 자잘한
  "수수료도 못 건지는" 거래 자체를 줄이는 쪽으로 재조정한 지금 조합이
  네 조합 전부에서 승률·수익률 동시에 더 낫다.

  거래 표본이 수만 건이라 "원금 100% 복리"로 계산하면 숫자가 비현실적으로
  부풀므로, 실제로는 거래당 계좌 자본의 일부(1~5%)만 리스크에 거는
  자금관리가 필수다(수수료 반영 기준, 5년+ 전체 구간 — BTC 15분봉 1%
  노출 시 +99%/2%면 +297%/5%면 +2,908%, BTC 5분봉은 1%면 +83%/2%면
  +235%/5%면 +1,945%, ETH 15분봉 1%면 +123%/2%면 +398%/5%면 +5,396%,
  ETH 5분봉 1%면 +61%/2%면 +159%/5%면 +980%, 전부 MDD 5% 이내) — 자세한
  근거는 `app/lab_strategies.py`의 `BollingerWickBreakevenTrailStrategy`
  docstring.
