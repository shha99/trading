...
- **실계좌 전에 반드시 테스트넷에서 검증한다** (`BINANCE_TESTNET=true`가
  기본값이다).
- **공개 도메인(Render 등)에 배포했다면 `DASHBOARD_USERNAME`/`DASHBOARD_PASSWORD`를
  설정해 대시보드에 비밀번호를 걸 것을 권장한다.** 실계좌 매매 기록·열린
  포지션·리스크 현황이 다 보이는 화면이라, URL만 알면 누구나 들어올 수 있는
  상태로 두지 않는 게 안전하다. 둘 다 채워야 켜지고(하나라도 비면 인증 없이
  열림 - 기본값), 켜면 페이지·API·정적파일 전체(`/ws/live` 웹소켓 포함)에
  HTTP Basic Auth가 걸린다.
