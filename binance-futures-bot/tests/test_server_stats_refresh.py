...
서버 전체를 띄우지 않고, 세 build_* 함수를 monkeypatch해서 호출 여부/순서와
...
def test_refresh_all_stats_calls_all_three_builders(monkeypatch):
    calls = []
    ...
    assert calls == ["strategy", "lab", "validated"]
...
def test_refresh_all_stats_skips_when_already_running(monkeypatch):
    ...
    assert calls == []  # lab/validated는 아예 호출 안 됐어야 함
