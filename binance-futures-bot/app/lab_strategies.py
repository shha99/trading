...
class BollingerWickBreakevenTrailStrategy(LabStrategy):
    ...
    **수수료 0.1%(바이낸스 선물 테이커 왕복) 반영, 5년 이상 전체 히스토리 기준**:
    ...
    - BTCUSDT 15분봉: 학습 88.1%/+0.448%, 검증 75.9%/+0.193%
    - BTCUSDT 5분봉: 학습 82.4%/+0.298%, 검증 55.8%/+0.091% (넷 중 마진이 가장 얇음)
    - ETHUSDT 15분봉(교차검증): 학습 82.2%/+0.676%, 검증 78.3%/+0.418%
    - ETHUSDT 5분봉(교차검증): 학습 75.8%/+0.341%, 검증 63.8%/+0.110%

    (구 파라미터 대비 네 조합 전부에서 수수료 반영 후 승률·거래당 수익률이
    동시에 개선됐다 — 특히 BTC 5분봉 검증 구간은 +0.059%→+0.091%로 약 54%
    개선. 정확한 최신 수치는 `data/validated_lab_stats.json`/`/api/lab/validated-stats`
    또는 README "시간대별 운용 방침" 참고.)
